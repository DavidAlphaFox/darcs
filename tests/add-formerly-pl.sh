...
darcs add foo.d
darcs add oof.d
not darcs add -v foo.d 2>&1 | grep -i directory
# Try adding the same directory when it's already in the repo
not darcs add -v foo.d oof.d 2>&1 | grep -i directories
...
darcs add bar
darcs add baz
not darcs add -v bar 2>&1 | grep -i "following file is"
not darcs add -v bar baz 2>&1 | grep -i "following files are"

# Make sure that messages about both files and directories say so
not darcs add -v bar foo.d 2>&1 | grep -i 'files and directories'
